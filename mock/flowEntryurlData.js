...
function getGroupSum(array,url,referType){
	return new EntryurlModel(array._first()._site_id, array._first()._report_time, 
        ...
        referType,url,
       array._sum(function (p) { return p._contribPv; }),array._sum(function (p) { return p._entryCount; }));
}
...
function getData(date){
    ...
    var reportTime=date._format('yyyy-MM-dd');
	    for (var i = 0; i < 100; i++) {
        var m = new EntryurlModel(config.siteId, reportTime, 
            randomNum(), randomNum(), randomNum(), randomNum(), randomNum(), randomNum(), randomNum(),
            randomRefer(),randomUrl(),randomNum(),randomNum());
        origin.push(m);
    }
    var result=[];
    var group=groupArray(origin,'_entry_url','_refer_type');
    for(var f1 in group){
        var obj=group[f1];
        for(var f2 in obj){
