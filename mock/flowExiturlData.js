...
var urlModel=require('../models/FlowExiturlModel.js')
...
function getGroupSum(array,url,referType){
	return new urlModel(array._first()._site_id, array._first()._report_time, 
        ...
        referType,url,
       array._sum(function (p) { return p._contribPv; }),array._sum(function (p) { return p._exitCount; }));
}
...
function getData(date){
    ...
    var result=[];
    var group=groupArray(origin,'_exist_url','_refer_type');
    for(var f1 in group){
        var obj=group[f1];
        for(var f2 in obj){
